...
def dynamic_import_class(file_path, class_name):
...
def load_class(path: str, file_name: str, class_name: str):
    for dirpath, _, filenames in os.walk(path):
        for filename in filenames:
            if filename.lower() == file_name.lower():
                for cl in get_classes(os.path.join(dirpath, filename)):
                    if cl.lower() == class_name.lower():
                        return dynamic_import_class(os.path.join(dirpath, filename), cl)
    return None


def load_entries_from_directory(directory: str) -> list[Entry]:
    ...
    for game in games[1]:
        ...
        game_class = load_class(game_path, f"{game}Game.py", f"{game}Game")
        wrapper_class = None
        if os.path.exists(game_path_pytorch):
            wrapper_class = load_class(game_path_pytorch, "NNet.py", "NNetWrapper")
        elif os.path.exists(game_path_keras):
            wrapper_class = load_class(game_path_keras, "NNet.py", "NNetWrapper")
...
class GameEnum(metaclass=GameEnumMeta):
    @classmethod
    def add(cls, key: str, value: Entry):
...
print(GameEnum)  # empty GameEnum
entries = load_entries_from_directory(directory="../GameClient/Games")
for entry in entries:
    GameEnum.add(entry.game_name, entry)
print(GameEnum)  # filled GameEnum
for g in GameEnum:
    print(g)  # iterate over GameEnum

print(GameEnum.connect4)  # direct access -> Error if not exist

print(GameEnum.get("COnnEct4"))  # get method ignore upper and lower characters
