...
class MCTS():
    ...
    def getActionProb(self, canonicalBoard, temp=1):
        ...
        probs = [x / counts_sum for x in counts] # [0.1, 0.2, 0.25, 0.05, 0.05, 0.1, 0.05, 0.05, 0.05, 0]; last one=flag
        return probs
    ...
    def search(self, canonicalBoard):
        ...
        s = self.game.stringRepresentation(canonicalBoard)  # s is the state (the node in the MCTS, the board (positions))

        if s not in self.Es:    # Es stores for every board state (node) whether game is ended (and who has won) or not
            self.Es[s] = self.game.getGameEnded(canonicalBoard, 1)  # if not ended (most cases), a 0 will be written for that state s
        ...
        if s not in self.Ps:  # if no policy already calculated for that state (board position) = leaf node
            ...
            valids = self.game.getValidMoves(canonicalBoard, 1)  # btw: its always player 1, because the board gets unified
                                                                 # (=canonical board)
            ...
            if sum_Ps_s > 0:
                ...
                # because the sum of all probabilities is 1 when all moves are valid (=1) => "self.Ps[s] = self.Ps[s] * valids"
