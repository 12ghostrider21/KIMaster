...
class Board():
                    ...
                    if self[x1][y1] == 0:
                        ...
                        for length2 in range(length + 1, 8):
                            ...
                            if self.is_on_board(cap_x, cap_y) and self[cap_x][cap_y] == 0:
                                ...
                                backward = (-direction[0], -direction[1])
                                captured = [(x1, y1)]
                                is_long_capture = self.has_additional_king_captures_for_square(cap_x, cap_y, backward,
                                                                                               captured, piece)
    ...
    def execute_move(self, move, color):
        ...
        if self.last_long_capture:
            ...
            try:
                assert last.x1 == move.x0 and last.y1 == move.y0, "Long capture must continue: " + self.display() + str(
                    last) + " != " + str(move)
            except:
                print("Hello")
    ...
    def execute_move_on_pieces(self, pieces, move, color):
        ...
        promotion = (move.y1 == 7 and pieces[move.x0][move.y0] == 1) or (
                    move.y1 == 0 and pieces[move.x0][move.y0] == -1)
    ...
    def display(self, file=None):
        ...
        print("rotation:", self.rotation, ", pieces:", self.count_pieces(), ", kings: (", self.whiteKingCount, ",",
              self.blackKingCount, ")",
...
class Move():
    ...
    def parse_action(action):
        ...
        assert 0 <= x0 and x0 < 8 and 0 <= x1 and x1 < 8 and 0 <= y0 and y0 < 8 and 0 <= y1 and y1 < 8, "Illegal action: " + str(
            action) + "=>" + str((x0, y0, x1, y1))
        m = Move(x0, y0, x1, y1, None)
        # m.is_long_capture = long_capture
        return m
