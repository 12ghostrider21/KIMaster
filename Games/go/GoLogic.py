...
class Board:
    ...
    def __init__(self, size: int = None, pieces: np.array = None):
        ...
        self.liberty_sets = [[set() for _ in range(self.size)] for _ in range(self.size)]
        ...
        self.hash_lookup = {
            WHITE: rng.randint(np.iinfo(np.uint64).max, size=(self.size, self.size), dtype='uint64'),
    ...
    def _create_neighbors_cache(self):
        if self.size not in Board.__NEIGHBORS_CACHE:
            Board.__NEIGHBORS_CACHE[self.size] = {}
            for x in range(self.size):
                for y in range(self.size):
                    neighbors = [xy for xy in [(x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)]
                                 if self._on_board(xy)]
                    Board.__NEIGHBORS_CACHE[self.size][(x, y)] = neighbors
    ...
    def _neighbors(self, position):
        ...
        """
        return Board.__NEIGHBORS_CACHE[self.size][position]
