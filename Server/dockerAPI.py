...
class DockerAPI:
    ...
    def startGameClient(self, token:str):
        ...
        docker_client.containers.run('game-client-img',             # image to run
                                     ...
                                     remove=True,                  # Set True to auto remove - Can make debugging harder, because container logs will be removed too
                                     environment= {                 # Set Environment Variables (override defaults from GameClient Dockerfile)
                                             'LOBBY_KEY' : token,
    ...
    def stopGameClient(self, token:str):
        ...
        try:
            ...
            print(f'Failed Stopping GameClient {token}: docker ran into an error, when trying to stop GameClient Container: {token}')
    ...
    def removeGameClient(self, token:str):
        ...
        try:
            ...
            print(f'Failed Removing GameClient {token}: docker ran into an error, when trying to remove GameClient Container: {token}')
